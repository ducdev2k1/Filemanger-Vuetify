// Types
import type { UnionToIntersection } from '@/utils/Vuetify';
import type { ComponentOptionsBase, ComponentPublicInstance, Ref, UnwrapRef } from 'vue';

const Refs = Symbol('Forwarded refs');

/** Omit properties starting with P */
type OmitPrefix<T, P extends string, E = Extract<keyof T, `${P}${any}`>> = [E] extends [never]
  ? T
  : Omit<T, `${P}${any}`>;
  ...
  return new Proxy(target, {
    ...
    getOwnPropertyDescriptor(target, key) {
      ...
      for (const ref of refs) {
        ...
        const descriptor =
          getDescriptor(ref.value, key) ?? ('_' in ref.value ? getDescriptor(ref.value._?.setupState, key) : undefined);
        if (descriptor) return descriptor;
      }
