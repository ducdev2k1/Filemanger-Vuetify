...
// Utilities
import { parseAnchor, propsFactory } from '@/utils/Vuetify';
import { computed } from 'vue';

// Types
import type { Anchor } from '@/utils/Vuetify';
...
export function useLocation(props: LocationProps, opposite = false, offset?: (side: string) => number) {
  ...
  const locationStyles = computed(() => {
    ...
    const { side, align } = parseAnchor(
      props.location.split(' ').length > 1 ? props.location : (`${props.location} center` as Anchor),
      isRtl.value,
    );
