...
export function useAspectStyles(props: { aspectRatio?: string | number }) {
  return {
    aspectStyles: computed(() => {
      ...
      return ratio ? { paddingBottom: String((1 / ratio) * 100) + '%' } : undefined;
    }),
  };
}
...
export const VResponsive = genericComponent<VResponsiveSlots>()({
  ...
  setup(props, { slots }) {
    ...
    useRender(() => (
      <div
        class={['v-responsive', { 'v-responsive--inline': props.inline }, props.class]}
        style={[dimensionStyles.value, props.style]}>
        <div class="v-responsive__sizer" style={aspectStyles.value} />

        {slots.additional?.()}

        {slots.default && <div class={['v-responsive__content', props.contentClass]}>{slots.default()}</div>}
      </div>
    ));
