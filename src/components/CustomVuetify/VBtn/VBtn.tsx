...
// Components
import { VBtnToggleSymbol } from '@/components/CustomVuetify/VBtnToggle/VBtnToggle';
import { VDefaultsProvider } from '@/components/CustomVuetify/VDefaultsProvider';
import { VIcon } from '@/components/CustomVuetify/VIcon';
import { VProgressCircular } from '@/components/CustomVuetify/VProgressCircular';
...
export const VBtn = genericComponent<VBtnSlots>()({
  ...
  setup(props, { attrs, slots }) {
    ...
    const variantProps = computed(() => {
      const showColor =
        (group?.isSelected.value && (!link.isLink.value || link.isActive?.value)) || !group || link.isActive?.value;
      return {
    ...
    function onClick(e: MouseEvent) {
      if (
        isDisabled.value ||
        (link.isLink.value && (e.metaKey || e.ctrlKey || e.shiftKey || e.button !== 0 || attrs.target === '_blank'))
      )
        return;
    ...
    useRender(() => {
      ...
      return withDirectives(
        ...
        [[Ripple, !isDisabled.value && props.ripple, '', { center: !!props.icon }]],
      );
    });
